...
def init_knowledge_base():
    kb = KnowledgeBase(
        "mongodb+srv://rabirabi:Rabirabi80@cluster0.ylk5353.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
    )
...
if menu == "Diabetes Information":
    ...
    if submenu == "Ask the Question":
        ...
        st.write(
            """Our system uses fuzzy logic to match your questions with the most relevant information available. 
...
if menu == "Model Performance":
        ...
        with col1:
            st.plotly_chart(
                plot_confusion_matrix(
                    train_metrics["Confusion Matrix"], "Training Confusion Matrix"
                )
            )
        with col2:
            st.plotly_chart(
                plot_confusion_matrix(
                    val_metrics["Confusion Matrix"], "Validation Confusion Matrix"
                )
            )
    ...
    with st.expander("Feature Importance"):
        ...
        feature_imp = pd.DataFrame(
            sorted(zip(importances, x_train.columns)), columns=["Value", "Feature"]
        )
        fig = px.bar(
            feature_imp,
            x="Value",
    ...
    with st.expander("Error Analysis"):
        ...
        feature = st.selectbox(
            "Select feature for misclassification analysis", x_val.columns
        )
        fig = px.histogram(
            misclassified,
            x=feature,
...
if menu == "Data Exploration":
    ...
    fig = px.pie(
        ...
        names=y_train.value_counts().index,
        title="Distribution status Diabetes",
    )
    st.plotly_chart(fig)
...
if menu == "Predict Diabetes":
    ...
    if "combined_risk" not in st.session_state:
        st.session_state.combined_risk = 0

    with st.form("prediction_form"):
        st.header("Input Patient Data")
        glucose = st.number_input("Glucose", min_value=0, max_value=200, value=117)
        blood_pressure = st.number_input(
            "Blood Pressure", min_value=0, max_value=122, value=72
        )
        skin_thickness = st.number_input(
            "Skin Thickness", min_value=0, max_value=99, value=23
        )
        ...
        diabetes_pedigree_function = st.number_input(
            "Diabetes Pedigree Function", min_value=0.0, max_value=2.42, value=0.3725
        )
    ...
    if submitted:
        ...
        combined_risk, model_risk, fuzzy_risk = predict_diabetes_with_fuzzy(
            input_data, model, fuzzy_system
        )

        # Save the states
        st.session_state.prediction_made = True
        st.session_state.combined_risk = combined_risk
        st.session_state.model_risk = model_risk
        st.session_state.fuzzy_risk = fuzzy_risk
        dynamic_message = DynamicMessage()
        if combined_risk < 50:
            st.session_state.message = dynamic_message.get_success_message(
                bmi, age, glucose
            )
        else:
            st.session_state.message = dynamic_message.get_warning_message(
                bmi, age, glucose
            )
    ...
    if st.session_state.prediction_made:
        if not st.session_state.audio_generated:
            speech = text_to_speech(st.session_state.message)

            st.subheader("Prediction Result")
            st.write(f"Combined Risk: {st.session_state.combined_risk:.2f}%")
            st.write(f"Random Forest Model Risk: {st.session_state.model_risk:.2f}%")
            st.write(f"Fuzzy Logic Risk: {st.session_state.fuzzy_risk:.2f}%")
            ...
            fig = go.Figure(
                go.Bar(
                    ...
                    y=[
                        st.session_state.combined_risk,
                        st.session_state.model_risk,
                        st.session_state.fuzzy_risk,
                    ],
                    text=[
                        f"{st.session_state.combined_risk:.1f}%",
                        f"{st.session_state.model_risk:.1f}%",
                        f"{st.session_state.fuzzy_risk:.1f}%",
                    ],
                    textposition="auto",
                )
            )
            ...
            st.plotly_chart(fig)
            st.info(
                "Note: This prediction combines machine learning (Random Forest) with fuzzy logic for a more nuanced assessment. The fuzzy logic system takes into account the imprecision in medical data and diagnostic boundaries."
            )
